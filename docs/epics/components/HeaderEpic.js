...
const HeaderEpic = ({ zoom, setZoom, setIsSettingsOpen, fileInputRef, importData, takeSnapshot, addTask }) => {
    return html`
        <header class="bg-white border-b px-6 py-4 flex items-center justify-between shadow-sm" style="position: sticky; top: 0; left: 0; z-index: 50;">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-xl font-bold tracking-tight text-slate-900 font-serif italic">Epics Planner</h1>
            </div>

            <div class="flex items-center gap-4">
                <div class="flex items-center bg-slate-100 rounded-lg p-1 border border-slate-200">
                    <button onClick=${() => setZoom(Math.max(0.2, zoom - 0.2))} class="px-3 py-1 hover:bg-white rounded-md transition-all text-xs font-bold active:scale-95">Zoom Out</button>
                ...
                <div class="flex gap-2">
                    ...
                    <input type="file" ref=${fileInputRef} style="display: none" accept=".csv" onChange=${importData} />
                    ...
                    <button onClick=${addTask} class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg text-sm font-bold transition-all shadow-md active:scale-95">+ Add Epic</button>
                </div>
            </div>
        </header>
    `;
